...
\subsection{语义安全(Semantic Security)}
在本节中，我们定义公钥加密系统的第二个安全标准，称为语义安全。非正式地说，如果窃听者可以根据给定的密文计算出明文，他也可以在没有密文的情况下进行计算，那么系统在语义上是安全的。我们证明了每一个多项式安全的公钥密码系统都是语义安全的。因此，概率PKC在语义上是安全的。因此，我们的加密方案通过了Shannon[23]完美保密定义的多项式有界版本:将我们的注意力限制在具有多项式有界资源可用于分析被截获消息的对手上，代表各种消息的被截获密码的后验概率与被截获前相同消息的先验概率相同。
